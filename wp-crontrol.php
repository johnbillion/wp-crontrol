...
if ( ! class_exists( 'Crontrol' ) ) {
	...
	class Crontrol {
		 ...
		 *
		 * @access public
		 * @param mixed $to_delete To Delete.
		 * @param mixed $sig Signature.
		 * @param mixed $next_run Next Run.
		 */
				...
				foreach ( $schedules as $name => $data ) {
					...
					if ( in_array( $name, $custom_keys, true ) ) {
						if ( in_array( $name, $used_schedules, true ) ) {
							...
							printf(
								'<span class="row-actions visible"><span class="delete"><a href="%s">%s</a></span></span>',
								esc_url( $url ),
		 ...
		 *
		 * @access public
		 * @param bool $cache (default: true) Cache. Cached for one hour when all is well.
		 */
		...
		public function show_cron_form( $is_php, $existing ) {
			...
			if ( is_array( $existing ) ) {
				...
				$other_fields .= sprintf(
					'<input name="original_hookname" type="hidden" value="%s" />',
					esc_attr( $existing['hookname'] )
				);
				$other_fields .= sprintf(
					'<input name="original_sig" type="hidden" value="%s" />',
					esc_attr( $existing['sig'] )
				);
				$other_fields .= sprintf(
					'<input name="original_next_run" type="hidden" value="%s" />',
		...
		<div id="crontrol_form" class="wrap narrow">
			<h2 class="nav-tab-wrapper">
				<a href="<?php echo esc_url( $new_links['cron'] ); ?>" class="nav-tab
									<?php
									if ( ! $show_edit_tab && ! $is_php ) {
										echo ' nav-tab-active'; }
									?>
				"><?php echo esc_html( $new_tabs['cron'] ); ?></a>
				<a href="<?php echo esc_url( $new_links['php-cron'] ); ?>" class="nav-tab
									<?php
									if ( ! $show_edit_tab && $is_php ) {
										echo ' nav-tab-active'; }
									?>
